...
function PANEL:Init()
    ...
    self.Deals:SetMouseInputEnabled(true)
...
function PANEL:InitSeason()
    ...
    for y = 1, 2 do
        for x = 1, 2 do
            ...
            item.Buy:SetText("")
            item.Buy.DoClick = function()
                if (itemData.credits or 0) > LocalPlayer():getCredits() then
                    Derma_Message("You cannot afford this item!", "NebulaRP", "OK")
                    return
                end

                Derma_Query("Are you sure do you want to buy this item? You cannot refund it!", "Confirm", "Yes", function()
                    net.Start("NebulaRP.StoreBuy")
                    net.WriteUInt(2, 3)
            ...
            item.Image:SetMouseInputEnabled(true)
